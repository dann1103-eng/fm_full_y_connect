...
export function NotificationsDropdown() {
  ...
  function handleItemClick(item: NotificationItem) {
    ...
    if (item.kind === 'matrix_blocked' && item.matrix_id) {
      router.push(`/matrices/${item.matrix_id}`)
      return
    }
  ...
  if (item.kind === 'matrix_blocked') {
    ...
    return (
      <div className="relative group">
        ...
        >
          <span className="w-8 h-8 rounded-full bg-amber-100 dark:bg-amber-400/20 flex items-center justify-center flex-shrink-0">
            <span className="material-symbols-outlined text-[18px] text-amber-600 dark:text-amber-400">grid_view</span>
          </span>
          <div className="flex-1 min-w-0 pr-5">
            <div className="text-xs leading-tight">
              <span className="font-bold text-amber-700 dark:text-amber-400 uppercase tracking-wide text-[10px]">
              ...
              {item.matrix_title && (
                <div className="text-fm-on-surface-variant/70 text-[11px] truncate">{item.matrix_title}</div>
              )}
              <div className="text-fm-on-surface-variant/50 text-[10px] mt-0.5">{timeAgo}</div>
            </div>
          </div>
