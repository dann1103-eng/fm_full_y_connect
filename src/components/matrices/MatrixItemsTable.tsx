...
export function MatrixItemsTable({ items, usage, problems, selectedId, readOnly, onSelect, onAdd, onDuplicate, onDelete }: Props) {
  ...
  const rowActions = (it: ContentMatrixItem) => !readOnly && (
    <span className="inline-flex items-center gap-1">
      <button type="button" onClick={(e) => { e.stopPropagation(); onDuplicate(it.id) }} title="Duplicar pieza" aria-label="Duplicar pieza"
        className="material-symbols-outlined text-[18px] p-1 rounded-md text-fm-on-surface-variant hover:text-fm-primary">content_copy</button>
      <button type="button" onClick={(e) => { e.stopPropagation(); onDelete(it.id) }} title="Eliminar pieza" aria-label="Eliminar pieza"
  ...
  const flags = (it: ContentMatrixItem) => {
    ...
    return (
      <span className="flex flex-wrap items-center gap-1">
        {it.needs_production && <span title="Necesita producción" aria-label="Necesita producción" className="material-symbols-outlined text-[16px] text-fm-primary">videocam</span>}
              ...
              <tbody>
                {items.map((it) => (
                  <tr key={it.id} onClick={() => onSelect(it.id)}
                    ...
                    <td className="py-2.5 pr-3 text-fm-on-surface max-w-[20rem] truncate">{it.title || untitled}</td>
          ...
          <div className="sm:hidden space-y-2">
            {items.map((it) => (
              <div key={it.id} onClick={() => onSelect(it.id)}
                className={`rounded-xl border p-3 space-y-1.5 cursor-pointer ${
                ...
                <p className="text-sm text-fm-on-surface">{it.title || untitled}</p>
