...
export function MessageComposer({ conversationId, placeholder }: MessageComposerProps) {
  ...
  return (
    <div className="p-4 border-t border-[#dfe3e6] bg-white">
      <div className="bg-[#f5f7f9] rounded-lg border border-[#dfe3e6] p-2">
          ...
          <div className="flex flex-wrap gap-2 px-1 pb-2">
            {pendingFiles.map((p) => (
              <div
                ...
                className={cn(
                  'flex items-center space-x-2 bg-white border rounded-full px-3 py-1 text-xs',
                  p.error ? 'border-[#b31b25] text-[#b31b25]' : 'border-[#dfe3e6] text-[#595c5e]'
                )}
              >
                <svg className="w-3.5 h-3.5 text-[#00675c]" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M16.5 6v11.5c0 2.21-1.79 4-4 4s-4-1.79-4-4V5c0-1.38 1.12-2.5 2.5-2.5s2.5 1.12 2.5 2.5v10.5c0 .55-.45 1-1 1s-1-.45-1-1V6H10v9.5c0 1.38 1.12 2.5 2.5 2.5s2.5-1.12 2.5-2.5V5c0-2.21-1.79-4-4-4S7 2.79 7 5v12.5c0 3.04 2.46 5.5 5.5 5.5s5.5-2.46 5.5-5.5V6h-1.5z" />
                </svg>
                <span className="max-w-[180px] truncate">{p.file.name}</span>
                {p.uploading && <span className="text-[10px] text-[#00675c]">subiendo...</span>}
                <button
                  onClick={() => removeFile(p.file)}
                  className="text-[#595c5e]/60 hover:text-[#b31b25]"
                  type="button"
                  aria-label="Quitar"
                >
        ...
        <div className="flex items-end space-x-2">
          ...
          <button
            ...
            className="p-2 text-[#595c5e] hover:text-[#00675c]"
          ...
          <button
            ...
            onClick={send}
            disabled={sending}
            className="p-2.5 bg-[#00675c] text-white rounded-lg hover:opacity-90 disabled:opacity-50"
            title="Enviar"
          >
            <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
      ...
      <div className="mt-1 flex items-center justify-between text-[10px] text-[#595c5e]/70 px-1">
        <span>
          <b>Enter</b> para enviar · <b>Shift+Enter</b> nueva línea · Máx. 10 MB por archivo
        </span>
        {error && <span className="text-[#b31b25]">{error}</span>}
      </div>
    </div>
  )
}
