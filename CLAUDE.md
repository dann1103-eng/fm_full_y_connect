...
- `/admin/whatsapp` (solo admin) — tabs Clientes / Leads para editar prompt + tools por audience. Stats de consumo del mes actual y anterior + breakdown por cliente. Sección **`WaBotFailedJobs`**: últimos 20 `ai_jobs` con `status='failed'` **filtrados a `whatsapp_reply`, `whatsapp_template` e `invoice_due_reminder`** (`.in('job_type', …)`) con cliente, factura, intentos y error. El filtro no es opcional: una generación de matriz con límite de tasa mete 15+ `matrix_item_write` fallidos y sin él empujaría fuera de las 20 los recordatorios de factura; los fallos de matrices se ven en el editor de la matriz. Es la única superficie donde el equipo ve que algo del bot no salió — un recordatorio de factura se envía at-most-once, así que si falla el cliente no recibe aviso.
...
- Modelo en `src/lib/ai/matrix/model.ts`: **`ANTHROPIC_MATRIX_MODEL`** `|| 'claude-sonnet-4-6'` — variable **propia**: `ANTHROPIC_MODEL` es la del bot de WhatsApp y cambiarla no debe alterar las matrices. Padre `max_tokens 4000`/`temperature 0.7`, hijo `1500`/`0.8`. Sin `ANTHROPIC_API_KEY` el handler lanza: el job termina `failed` y se ve **en el editor de la matriz** (franja roja del padre o "No se pudo redactar" en la pieza), no en `WaBotFailedJobs`, que solo lista el bot y los recordatorios.
...
- Route handler y no server action (el sondeo necesita `AbortSignal`). **Sesión normal: NO va en `SECRET_AUTH_API_PREFIXES`** (esos son los crons). Re-valida admin/supervisor y el `id` como uuid (`UUID_RE` de `matrix.ts` → **400**, que apaga el sondeo; sin eso Postgres devolvía `invalid input syntax for type uuid` como un 500 que el hook reintentaba para siempre); lee `ai_jobs` con el **admin client** (su única policy de `select` es `is_admin()`, un supervisor no pasa) y las tablas de matrices con el de sesión.
- **Solo el padre más reciente** decide `phase` (`planning`/`writing`/`idle`/`failed`), `error` y `reason`; los contadores `total`/`done`/`failed` son de **sus** hijos (un hijo `completed` sin escribir cuenta como fallido, salvo `ya_redactada`/`editada_a_mano`, que cuentan como hechos); los `matrix_item_write` sueltos de "Regenerar" solo alimentan `writingItemIds`. Además: `itemJobs` (el **último** hijo de cada pieza: la entrada de `generationGate`), `failedItems` (piezas cuyo último hijo falló o terminó sin escribir, con el `skipped` o el `error_text`), `topics` + `matrixUpdatedAt`, `items` con `updated_at > since` y **`watermark`**. La forma (`GenerationProgress`) vive en `src/lib/domain/matrix-generation.ts`.
- **La ruta lee el padre ANTES que la matriz**: al revés, un padre que terminara entre las dos lecturas saldría con una fase que ya no es `planning` junto a los temas viejos, y el editor desbloquearía la barra de temas con la lista vacía.
...
- **Regla de fusión** (`src/lib/domain/matrix-generation.ts`, con tests), sobre la máquina de estados del bloque 1 sin debilitarla:
  ...
  - **Los temas** del sondeo van a `matrix` **y** a `confirmedMatrix` (con `matrixUpdatedAt` contra la versión confirmada). Sin esto la barra seguiría con la lista vieja y el siguiente cambio de temas mandaría a `updateMatrix` una lista **sin** los de la IA: el servidor los borraría y soltaría el `topic` de todas las piezas generadas. Mientras un guardado de temas vuela, el `topic` de toda pieza espera. **Mientras el padre planifica (`phase === 'planning'`) la barra de temas queda bloqueada** con el motivo a la vista (`topicsLockedReason` de `MatrixTopicsBar`; el enfoque del mes sigue editable): el padre indexa sus temas sobre la lista que leyó, o escribe la suya si estaba vacía, y un cambio en esa ventana dejaría piezas con el tema equivocado.
