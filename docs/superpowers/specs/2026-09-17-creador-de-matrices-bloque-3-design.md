...
**Corrección posterior (revisión final):** la guarda solo corta antes de *reclamar*, así que un job reclamado a los 44,9 s seguía muriendo con `maxDuration = 60` (un padre con un plan grande pasa de 40–80 s de modelo), y si era su último intento quedaba `processing` para siempre: el watchdog de 0124 exige `attempts < max_attempts` y los índices únicos de 0131 cubren `processing`, así que la matriz o la pieza quedaban bloqueadas. Dos arreglos: `maxDuration = 180` en `/api/ai-jobs/process` (no más: cerca de 300 s el watchdog reclamaría dos veces un job vivo) y `sweepExhaustedZombies` al arrancar `runJobs`, que marca `failed` los `processing` con `locked_at` de más de 10 min y los intentos agotados.
